...
#define INPUT_BUF_SIZE 128
  char* buf;// used to be char buf[INPUT_BUF_SIZE]; //this buffer needs to be allocated
  uint r;  // Read index
  uint w;  // Write index
  uint e;  // Edit index
} cons;
...
consoleinit(void)
{
  ...
  cons.buf = (char*) kmalloc(INPUT_BUF_SIZE * sizeof(char));
  if (cons.buf == 0) panic("error: consoleinit");
